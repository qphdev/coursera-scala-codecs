...
trait DecoderInstances {
  ...
  // Define an implicit value of type `Decoder[Int]`
  ...
  // Define an implicit value of type `Decoder[String]`
  ...
  // Define an implicit value of type `Decoder[Boolean]`
  ...
  implicit def listDecoder[A](implicit decoder: Decoder[A]): Decoder[List[A]] =
    Decoder.fromFunction {
      case Json.Arr(arr) => {
        val decoded = arr.flatMap(decoder.decode)
        if (decoded.length == arr.length)
          Some(decoded)
        else None
      }
      case _ => None
...
trait ContactsCodecs {
  ...
  // Define the encoder and the decoder for `Contacts`
  ...
  implicit val contactsDecoder: Decoder[Contacts] =
    Decoder.field[List[Person]]("people").transform[Contacts](contacts => Contacts(contacts))
